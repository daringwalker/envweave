...
EnvWeave 是一个面向 Linux 与 macOS 的桌面环境迁移管理器。它帮助用户在重装系统或更换设备后，安全地收集、版本管理、同步和恢复配置文件与软件包。
...
当前版本为可运行的 `0.1.0-alpha.1` 开发者预览版，提供完整的本地迁移闭环和 Git/软件包恢复基础能力。Alpha 版本用于受控测试，不应在没有独立备份的环境中作为唯一恢复手段。
