  ...
  <main class="theme-showcase">
    ...
    <section class="qa-grid">
      ...
      <article class="panel qa-panel qa-wide">
        ...
        <div class="restore-steps qa-restore"><article><span class="step-state">可执行</span><div class="step-main"><strong>恢复 Zsh 配置</strong><code>~/.zshrc</code><small>目标目录存在并可写</small></div><ul><li>创建事务备份</li><li>原子替换文件</li></ul></article><article class="review"><span class="step-state">需确认</span><div class="step-main"><strong>恢复 KDE 面板布局</strong><code>plasma-org.kde.plasma.desktop-appletsrc</code><small>当前设备布局可能不同</small></div><label class="review-choice"><input type="checkbox">允许恢复</label></article><article class="blocked"><span class="step-state">阻塞</span><div class="step-main"><strong>恢复系统级配置</strong><code>/etc/environment</code><small>需要管理员权限</small></div><ul><li>保持当前文件</li></ul></article></div>
      </article>
